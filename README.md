...
**动效层（可关）**
- 一个独立的前卫层：胶片颞粒、十二栏栅格、消息行号、左栏竖排字标、
  实时钟与连接心跳、坐标十字准线，以及一整条「画布」式的会话切换
- 全部挂在 `html[data-fx="on"]` 下，关掉就回到上面那套克制的原版界面
- 切换方式：左栏底部的 `FX` 按钮、`Shift+F`，或网址后加 `?fx=off` / `?fx=on`
- 默认跟随系统的「减弱动态效果」偏好；窄屏会自动去掉占地方的那几样
